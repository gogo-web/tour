<template>
  <div>
    <div class="search">
      ...
      <input type="text" class="search-input" placeholder="请输入城市名（如北京、bj、beijing）" v-model="keyword" :class="[changeSearch]">
...
import {disresolve} from '../../../utils/utils.js'
export default {
  name: 'locationHeader',
  props:{
    cities: Object,
  ...
  methods:{
    ...
    loopWord(content){
      const citiesWrapper = this.cities
      for(let i in citiesWrapper){
        if(i.toLowerCase().indexOf(disresolve(content.substr(0,1)).substr(0,1))>-1){
          let arr = []
          citiesWrapper[i].forEach((value)=>{
  ...
  watch:{
    keyword(){
      ...
      if (content==='') {
        ...
        let result = []
        for (let i in this.cities) {
          let haveAlph = i.toLowerCase().indexOf(disresolve(content.substr(0,1)).substr(0,1))>-1
          if(haveAlph){
            this.cities[i].forEach((value)=>{
